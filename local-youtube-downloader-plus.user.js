...
// @version      0.9.33+
// @description        A fork of Local YouTube Downloader with various improvements.
...
;(function () {
	...
	const LOCALE = {
		en: {
			togglelinks: 'Show/Hide Video Download Links',
			stream: 'Stream',
			adaptive: 'Adaptive',
			videoid: 'Video ID: ',
        ...
        no: {
            ...
            adaptive: 'Adaptiv',
            videoid: 'Video ID: ',
            dlmp4: 'Last ned høy oppløsning mp4 med ett klikk',
        }
	}
	...
	const xhrDownloadUint8Array = async ({ url, contentLength }, progressCb) => {
		...
		const getBuffer = (start, end) =>
			new Promise((res, rej) => {
				...
				xhr.headers = {
					'User-Agent':
						'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.151 Safari/537.36',
